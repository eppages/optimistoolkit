...
class NoMigration extends AbstractPlacement {

    NoMigration(String nodeId, EmotiveOVF vmInfo, PlacementOptimizer po) {
        ...
        if(ObjectiveType.MAX_TRUST.equals(po.getMaximizationPolicy())) {
            ...
            try {
                trecValue = new Double(po.getTREC().ECO.forecastIPEcoefficiency(new Long(0), "energy"));
            } catch(Exception e) {
            ...
            try {
                trecValue = new Double(po.getTREC().ECO.forecastIPEcoefficiency(new Long(0), "ecological"));
            } catch(Exception e) {
                Log.getLogger().error("ECO.forecastIPEcoEfficiencyVMDeploymentKnownPlacement() failed: " + e.getMessage());
            }
    ...
    public String toString() {
        return "Keeping " + vmInfo.getId() + " at its current node (Utility: " + getUtility() + ")";
    }
}

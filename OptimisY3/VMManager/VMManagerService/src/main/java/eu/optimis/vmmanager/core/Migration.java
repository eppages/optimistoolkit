...
class Migration extends AbstractPlacement {
    protected List<String> activeNodes = null;

    public Migration(String destinationNode, EmotiveOVF vmInfo, List<String> activeNodes, PlacementOptimizer po) {
        super(destinationNode, vmInfo, po);
        ...
        if(ObjectiveType.MAX_TRUST.equals(po.getMaximizationPolicy())) {
            ...
            try {
                trecValue = new Double(po.getTREC().ECO.forecastIPEcoEfficiencyVMMigrationKnownPlacement(vmInfo.getId(), destinationNode, activeNodes, "energy" , new Long(0)));
            } catch(Exception e) {
            ...
            try {
                trecValue = new Double(po.getTREC().ECO.forecastIPEcoEfficiencyVMMigrationKnownPlacement(vmInfo.getId(), destinationNode, activeNodes, "ecological" , new Long(0)));
            } catch(Exception e) {
                Log.getLogger().error("ECO.forecastIPEcoEfficiencyVMDeploymentKnownPlacement() failed: " + e.getMessage());
            }
    ...
    public String toString() {
        return "Migration of " + vmInfo.getId() + " to node " + nodeId + " (Utility: " + getUtility() + ")";
    }
}

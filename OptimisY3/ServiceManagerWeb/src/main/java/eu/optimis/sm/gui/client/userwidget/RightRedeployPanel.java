...
public class RightRedeployPanel extends ContentPanel {
	...
	public RightRedeployPanel() {
		setHeading("Redeploy service among the available services:");
		setLayout(new FitLayout());
		
		ToolBar toolBar = new ToolBar();
		Button redeployButton = new Button("Redeploy");
		checkBox = new CheckBox();
		Label label = new Label("Keep data:");
		checkBox.setValue(true);
		checkBox.setTitle("Keep data");
		redeployButton.setIcon(IconHelper.createStyle("icon-email-add"));
		redeployButton.setShadow(true);
		redeployButton.setBorders(true);
		id = new TextField<String>();
		id.setFieldLabel("service ID");
		id.setAllowBlank(false);
		toolBar.add(new LabelToolItem("Service number: "));
		toolBar.add(id);
		toolBar.add(redeployButton);
		toolBar.add(label);
		...
		refreshButton.addSelectionListener(new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				...
				removeAll();
				store2 = new String("Waiting for available services for redeploy response...");
				...
				service.availableServices(LeftPanelLogin.session_id, true,
						new AsyncCallback<ArrayList<Service>>() {
							public void onFailure(Throwable caught) {
								System.out.println("Error: available services for redeploy");
								Dispatcher.forwardEvent(MainEvents.error, caught);
							}
							public void onSuccess(ArrayList<Service> result) {
								System.out.println("Successfully executed: available services for redeploy");
		...
		redeployButton.addSelectionListener(new SelectionListener<ButtonEvent>() {
			@Override
			public void componentSelected(ButtonEvent ce) {
				...
				service.redeployService(LeftPanelLogin.session_id, serviceId, checkBox.getValue(),
						new AsyncCallback<String>() {
							public void onFailure(Throwable caught) {
								System.out.println("Error: redeploy");
								Dispatcher.forwardEvent(MainEvents.error, caught);
							}
							public void onSuccess(String result) {
								System.out.println("Successfully executed: redeploy");
								removeAll();
								store2 = new String("Redeployment of service with id = " + serviceId + ":\n result = "+ result);
								((TextArea)grid2).setValue(store2);
								add(grid2);
								layout(true);
							}

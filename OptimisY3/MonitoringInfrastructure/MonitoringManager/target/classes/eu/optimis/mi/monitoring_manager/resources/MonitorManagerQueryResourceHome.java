 ...
 * Monitoring Manager provides functionalities to gather monitoring data
 * from different monitoring connectors.
 * 
 * MonitoringManagerQueryResourceHome makes it possible to access the
 * monitoring data from multiple IPs related to a service.
 * 
...
public class MonitorManagerQueryResourceHome {
	...
	public MonitorManagerQueryResourceHome() {
		...
		try {
			String filename = "MultipleIps.log";
			String path = "hlrs/" + filename;
	...
	/**
	 * Run all the get* methods (with the exception of getLatestComplete*Resource methods) on multiple IPs.
	 * @param serviceId
	 * @param methodName
	 * @param parameters
	...
	public MonitoringResourceDatasets getFromMultipleIPs(
		...
		// Get list of physical node ids and list of IP addresses needed to call the endpoints.
	...
	/**
	 * Returns IP addresses associated with the service id passed as input argument.
	 * @param serviceId
	...
	public String getVMIPs(@PathParam("serviceId") String serviceId) {
		...
		CloudOptimizerRESTClient co = new CloudOptimizerRESTClient();
		List<String> vmsList = co.getVMsIdsOfService(serviceId);
		if (vmsList == null) {return "empty";}
		else {
			List<String> ipAddresses = getMultipleIpMap(vmsList).get("ipAddresses");
			String rs = array2Str(vmsList, ",") + " ; "	+ array2Str(ipAddresses, ",");
			return rs;
		}
	}
	...
	private static String getIPVmIpAddress(String infrastructureProviderId) {
		...
		if (infrastructureProviderId == null || infrastructureProviderId.length() < 1) {
			return null;
		}
		java.sql.Connection conn = MManagerDBUtil.getConnection("extern1");
		String ipAdd = InfrastructureProviderQuery.getVmIpAddress(conn,
	...
	/**
	 * @param: VM list
	 * @return: hashmap having 2 lists: one list called "ipAdresses", and one list called "nodes" that lists the physical node ids associated with the VMs
	 */
	private HashMap<String, List<String>> getMultipleIpMap(List<String> vmList) {
		...
		for (String vm : vmList) {
			...
			if (!nodesList.contains(physicalId)) nodesList.add(physicalId);
			if (!ipAddressList.contains(ipAddress) && ipAddress!=null) ipAddressList.add(ipAddress);
		}
	...
	private String array2Str(List<String> coll, String delimiter) {
		...
		for (String x : coll) sb.append(x + delimiter);

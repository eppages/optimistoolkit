...
public class getClient {
	...
	// [17] extended [12] QueryResource/date/metric/{metricName}/{resourceType}/{id}/{from}.{to}/
	//[12] QueryResource/date/metric/{metricName}/{resourceType}/{from}.{to}
	public MonitoringResourceDatasets getReportForPartMetricNameId(String metricName, String resourceType, String id, Date from, Date to) {
		SimpleDateFormat sdf = new SimpleDateFormat(dateformat);
		String sf = sdf.format(from);
		String st = sdf.format(to);
		String date = sf+"."+st;
		WebResource service = client.resource(this.getAddress(INTERFACE_4))
				.path("metric").path("id").path(metricName).path(resourceType).path(id).path(date);
		MonitoringResourceDatasets dataSet = service
				.get(MonitoringResourceDatasets.class);
		return dataSet;
	}
	
	// [18] extended [9] QueryResources/group/metric/id/{metricName}/{resourceType}/{resourceId}
	//[9] QueryResources/group/metric/{metricName}/{resourceType}
	public MonitoringResourceDatasets getLatestReportForMetricNameId(String metricName, String resourceType, String resourceId) {
		WebResource service = client.resource(this.getAddress(INTERFACE_3))
				.path("metric").path("id").path(metricName).path(resourceType).path(resourceId);
		MonitoringResourceDatasets dataSet = service.accept(MediaType.APPLICATION_XML)
				.get(MonitoringResourceDatasets.class);
		return dataSet;
	}

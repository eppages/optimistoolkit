delimiter //
DROP PROCEDURE IF EXISTS cal_mean_value_virtual_[TABLENAME] //

CREATE PROCEDURE cal_mean_value_virtual_[TABLENAME](IN metricname VARCHAR(45), IN year INT, IN month INT)
BEGIN	
 	DECLARE done INT DEFAULT FALSE;
	DECLARE rowId_var VARCHAR(45); 	
	DECLARE metricName_var VARCHAR(45);
	DECLARE meanValue_var VARCHAR(45);
	DECLARE phy_resourceId_var VARCHAR(45);
	DECLARE vir_resourceId_var VARCHAR(45);
	DECLARE col_id_var VARCHAR(45);
	DECLARE metricUnit_var VARCHAR(45);
	DECLARE stp INT(11);
        DECLARE check_mean_val INT;
        -- Calculate mean values.
	DECLARE cal_cursor CURSOR FOR SELECT metric_name, AVG(metric_value), physical_resource_id, virtual_resource_id,
                                      ...
                                      GROUP BY physical_resource_id, virtual_resource_id;
        ...
        THEN
           ...
           read_loop:LOOP	
	      FETCH cal_cursor INTO metricName_var, meanValue_var, phy_resourceId_var, vir_resourceId_var,col_id_var, metricUnit_var;
              ...
              VALUES(rowId_var, phy_resourceId_var, vir_resourceId_var, null, col_id_var, metricName_var, meanValue_var, metricUnit_var, stp);
           END LOOP;
           CLOSE cal_cursor;
...
CALL cal_mean_value_virtual_[TABLENAME]([METRICNAME], [YEAR], [MONTH]) //

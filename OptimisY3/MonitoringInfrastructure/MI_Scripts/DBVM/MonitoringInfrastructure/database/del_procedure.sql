...
BEGIN
  ...
  DECLARE cur_del CURSOR FOR SELECT row_id FROM [TABLENAME] WHERE date(from_unixtime(metric_timestamp)) <= date_sub(curdate(), INTERVAL p_month_offset MONTH) LIMIT 0, 1000;
  ...
  WHILE continueloop DO
     ...
     FROM [TABLENAME]
     WHERE date(from_unixtime(metric_timestamp)) <= date_sub(curdate(), INTERVAL p_month_offset MONTH);

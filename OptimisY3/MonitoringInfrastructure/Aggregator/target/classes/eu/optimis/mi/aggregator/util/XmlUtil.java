			...
			for (int i = 0; i < timestampList.getLength(); i++) {
				...
				try {
					...
					String timestamp = DateFormatUtils.ISO_DATETIME_FORMAT
							.format(udate);
					//System.out.println(tsLangType+"==>"+timestamp);
					ts.setTextContent(timestamp);
				} catch (NumberFormatException e) {
//					 SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
//					 String timestamp;
//					try {
//						timestamp = DateFormatUtils.ISO_DATETIME_FORMAT
//							.format(sdf.parse(tsLangType).getTime());
//						ts.setTextContent(timestamp);
...
//					}
	...
	// xml is orignial resources collected by a resource collector 
...
//					System.out.println(tp);
	...
	public void storeXmlResource(String xmlRes) {
		...
		try {
			...
			//InsertQuery.insertResources(DBUtil.getConnection(), mds);
	...
	public static void main(String[] args){
		String resource = 
			...
			"<metric_unit>percent</metric_unit><metric_timestamp>1231243"+ 
			"</metric_timestamp>" +
		 ...
		 MonitoringResourceDatasets mrs =  xmlu.getMRDObj(resource);
		 
		 for (MonitoringResourceDataset mds: mrs.getMonitoring_resource()){
			 System.out.println("=====================");

 ...
 *
 */
public class forcastVMCan {

    protected static Logger log = Logger.getLogger(physicalHost.class);

    //VMCancelled
    //for the physcal host we calculate and take the individual service levels away to see if improved risk
    //for the VM we calculate and take the individual service levels away to see if improved risk
    //for the SLA we use the new VM and Phy levels
    //for the IP we use the new SLA levels
    public static int[] calc(String replicationFactor, String VMID)
            throws ServletException, IOException, SQLException {
        int[] reVM = new int[20];
//		int[] phy = new int[1];

        log.info("vm replication = " + replicationFactor);

        //get current risk
        double[] risk2 = getRisk.calc();
        double[] riskFinal = vulnerabilities.calc(risk2);
        log.info("done get risk");



        /*We assume if the replication 
         * is less than 2 the risk will be highest. 
        ...
        if ((Integer.getInteger(replicationFactor) == null) || (Integer.getInteger(replicationFactor) < 2)) {
            log.info("low replication so recaclulating risk");
            reVM = recalcRisk();
        }

        //replace vm value in risk results before return
        riskFinal[1] = reVM[0];

        //convert risk to integer for return value
        int[] riskFinalInt = new int[riskFinal.length];
        int c = 0;

        while (c < riskFinal.length) {

            riskFinalInt[c] = (int) riskFinal[c];
            c++;
        }
    ...
    public static int[] recalcRisk() throws SQLException {
        ...
        float[] res2 = new float[6];
        float[] res3 = new float[6];
    ...
    public static int[] riskVM(String VMID) throws Exception {

        physicalHost.getData(VMID);
        ...
        con = DriverManager.getConnection(url, user, password);
        st = con.createStatement();
        rs = st.executeQuery("SELECT probability FROM `vulnerability` where VMID = " + VMID + ";");
        rs = st.executeQuery("SELECT AVE(probability) FROM `vulnerability`;");

        result[0] = rs.getFloat(1);
        int[] VMRisk = setBands(result, 1);
        return VMRisk;
    }
}

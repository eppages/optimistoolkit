...
public class GlobalConfiguration {
	...
	private void config() throws Exception{
		...
		if (defaultValues) {
			...
			try {
			    properties.load(new FileInputStream(this.configFilePath + File.separator + "config.properties"));
			} catch (IOException e) {
				LOGGER.error("The config.properties file does not exist at location: " + this.configFilePath + File.separator + "config.properties", e);
		...
		addRecontextFiles = Boolean.parseBoolean(properties.getProperty("addRecontextFiles"));
		LOGGER.info("Using addRecontextFiles value: '" + addRecontextFiles.toString() + "'");
	...
	/**
	 * @param addRecontextFiles the addRecontextFiles to set
	 */

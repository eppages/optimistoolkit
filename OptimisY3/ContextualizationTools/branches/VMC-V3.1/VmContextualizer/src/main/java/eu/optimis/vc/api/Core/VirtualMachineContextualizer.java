...
public class VirtualMachineContextualizer implements Runnable {
	...
	public void run() {
		...
		log.debug("Added new new progress object, initial progress is: "
				+ vmcApi.getGlobalState().getProgressData(serviceId)
						.getProgressString());
		log.info(vmcApi.getGlobalState().getProgressLogString(serviceId));

		// Five primary stages of contextualization...
		retriveContextualizationData();
		log.info(vmcApi.getGlobalState().getProgressLogString(serviceId));

		// Revert previously prepared VM images
		unprepareVmImages();
		log.info(vmcApi.getGlobalState().getProgressLogString(serviceId));
		...
		try {
			...
			log.error("Failed to convert VM image!", e);
		...
		try {
			...
			log.error("Failed to create ISO image!", e);
			vmcApi.getGlobalState().getProgressData(serviceId).setError(true);
			vmcApi.getGlobalState().getProgressData(serviceId).setException(e);
			return;
		}
		...
		vmcApi.getGlobalState().setProgressPercentage(serviceId, 100.0);
		log.info(vmcApi.getGlobalState().getProgressLogString(serviceId));
	...
	private void convertVmImages() throws SystemCallException {
		...
		if (imageFormat != null) {
			HashMap<String, VirtualMachine> virtualMachines = service
			...
			for (VirtualMachine virtualMachine : virtualMachines.values()) {
				...
				HashMap<String, HardDisk> hardDisks = virtualMachine
						.getHardDisks();
				...
				for (HardDisk hardDisk : hardDisks.values()) {
					...
					while (imageConversion.getSystemCall().getReturnValue() != 0) {
						...
						while (true) {
							...
							ArrayList<String> output = imageConversion
									.getSystemCall().getOutput();
							if (output.isEmpty() == true
									|| output.get(0).contains(")") == false) {
								if (imageConversion.getSystemCall()
										.getReturnValue() == -2) {
									log.error("SystemCall Error!");
									break;
								}
						...
						// total crap....
						if (imageConversion.getSystemCall().getReturnValue() == -2
						...
						log.debug("qemu progress value is: " + qemuProgress);
						double totalProgress = progress
								+ (((100.0 / virtualMachines.size()) / 100) * qemuProgress);
				...
				processsed++;
				progress = (100 / virtualMachines.size()) * processsed;
				vmcApi.getGlobalState().setProgressPercentage(serviceId,
						progress);
			}
			...
			log.info("Adding altered HardDisk URI's to manifest with id: "
	...
	private void createIsoImages() throws SystemCallException {
		...
		for (VirtualMachine virtualMachine : virtualMachines.values()) {
			...
			for (int i = 1; i <= virtualMachine.getUpperBound(); i++) {
				...
				log.info("Creating ISO with ID: " + iso.getImageId()
				...
				log.info("Completed creating ISO with URI: " + iso.getUri());

				// Calculate the progress...
				isosProcessed++;
				vmcApi.getGlobalState().setProgressPercentage(serviceId,
						(isosProcessed / isosToProcess) * 100);
			}
		}

		// Add the ISO URI's to the manifest
		log.info("Adding ISOs to manifest with id: " + manifest.getManifestId());
		ServiceManifestClient serviceManifestClient = new ServiceManifestClient(
				manifest);

...
public class VmcApi implements Api {
	...
	protected final static Logger log = Logger.getLogger(VmcApi.class);
	 ...
	 * the purpose of improving log readability.
	 * 
	 * @param manifestId
	...
	public void recontextualize(String hypervisorString) {
		try {
			...
			log.error("Failed to start Recontextualizer", e);
		}
	}
	...
	public void startRecontext(String domainName) {
			if (virtualMachineRecontextualizer != null) {
				try {
					...
					log.error("Failed to start Recontextualizer on specific domain", e);
				}
			}
	}

...
* 13. removed weight wc from gain in objective function
* 14. added storage constraints
...
* Memory additions
*-----------------------------------------------------
Parameter  free_mem(j)  'Reserved storage capacity'  /
$ondelim
$include free_mem.csv
$offdelim
/;
...
Parameter  K         Large constant                                        /10/;
Parameter  L         Very large constant                                /100000/;
...
Parameter  elastic_vms(i,a) New elastic VMs per service;
...
Table  gamma(i,a)
$ondelim
$include memory_per_component.csv
$offdelim;
display gamma;

Parameter elastic_memory(i,a) 'Maximum amount of storage';
elastic_memory(i,a)=gamma(i,a)*elastic_vms(i,a);
display elastic_memory;

Parameter basic_memory(i,a) 'Minimum amount of storage';
basic_memory(i,a)=gamma(i,a)*basic_vms(i,a);
display basic_memory;

Parameter  max_mem(a) Maximum storage per service;
           max_mem(a)= sum(i, basic_memory(i,a)) + sum(i, elastic_memory(i,a));

Display max_mem;
*-------------------------------------------------------------------------------


Parameter  max_av(a) maximum availability for service;
           max_av(a)= sum(i$vm(i,a), av(i,a))/num_comps(a);
...
Binary variables         xboolean(i,a) has service component i of service a used all basic vms,
                         ...
                         fed_vms_bool(i,a) are vms federated for the component or not,
                         cost_bool(a) does cost satisfy the constraint or not,
                         nn(i,a) don't ask;
...
Integer Variables        xx(j,i,a) how many vms of service component i are allocated for service a in host j,
                         zz(j,i,a) how much storage is allocated to component i of service a on host j,
                         ...
                         free_memory(j) available storage on host j;
...
Equations                obj, xbool_low(i,a), xbool_up(i,a), elastic_vms_eq(a), elastic_vms_per_comp_eq(i,a),
                         ...
                         cost_eq(a), fed_vm_eq_plus(i,a), affinity_rule_plus(a, combs_), new(i,a),

                         fed_vms_low(i,a), fed_vms_up(i,a), cdf_sc_eq_plus(i,a), cost_low(a), cost_up(a), tt(a), ttt(a),

                         new_admittance_(i,a), new_admittance(i,a), memory_res_Eq(j), zbool_low_(i,a), zbool_low(i,a),

                         zbool_up_(i,a), zbool_up(i,a), memory_alloc(j,i,a);
...
* residual storage per host
memory_res_Eq(j)..     free_memory(j) =e= free_mem(j) - sum((i,a)$vm(i,a),zz(j,i,a));
...
cdf_sc_eq(i,a)$(vm(i,a) and (1-doNotFederate(a)) and (Eco_s(a) gt eco_constraint(a)) and (Risk(a) lt risk_constraint(a)) and (Trust(a) gt trust_constraint(a)))..    cdf_sc(i,a)*(elastic_vms(i,a)+1)=e=sum(j, xx(j,i,a)) + fed_vms(i,a) - basic_vms(i,a)+1;
...
tt(a)$((Eco_s(a) lt eco_constraint(a)) or (Risk(a) gt risk_constraint(a)) or (Trust(a) lt trust_constraint(a)))..  sum(i,xboolean(i,a))=e=0;
...
fed_vms_eq(a)$(doNotFederate(a) or (Eco_s(a) lt eco_constraint(a)) or (Risk(a) gt risk_constraint(a)) or (Trust(a) lt trust_constraint(a)))..  sum(i,fed_vms(i,a))=e=0;
...
joint_cdf_eq(a)$no..                            joint_cdf(a)=e= prod(i$vm(i,a), 1-(1-cdf_sc(i,a))*xboolean(i,a));
...
prob_availability_Eq(a)$(doNotFederate(a) and no)..  joint_cdf(a)*accepted_vms_percent(a)=g=fi(a)*accepted_vms_percent(a);

* Boolean variables that signal if required availability is satisfied or not
joint_cdf_low(a)$(1-doNotFederate(a) and no)..    joint_cdf(a)-fi(a)=g=K*(joint_cdf_bool(a)-1);
joint_cdf_up(a)$(1-doNotFederate(a)  and no)..    joint_cdf(a)-fi(a)=l=K*joint_cdf_bool(a)-e;

*fed_vms_low(i,a)$(vm(i,a) and (1-doNotFederate(a)))..   fed_vms(i,a)-1=g=K*max_vms(a)*(fed_vms_bool(i,a)-1);
*fed_vms_up(i,a)$(vm(i,a) and (1-doNotFederate(a)))..    fed_vms(i,a)=l=K*max_vms(a)*fed_vms_bool(i,a);

fed_vms_low(i,a)$vm(i,a)..   fed_vms(i,a)-1=g=K*max_vms(a)*(fed_vms_bool(i,a)-1);
fed_vms_up(i,a)$vm(i,a)..    fed_vms(i,a)=l=K*max_vms(a)*fed_vms_bool(i,a);
...
zbool_low_(i,a)$((1-doNotFederate(a)) and vm(i,a))..   sum(j,zz(j,i,a))- gamma(i,a)*sum(j,xx(j,i,a))=g=K*max_mem(a)*(xboolean(i,a)-1);
zbool_up_(i,a)$((1-doNotFederate(a))  and vm(i,a))..   sum(j,zz(j,i,a))=l=K*max_mem(a)*xboolean(i,a);

memory_alloc(j,i,a)$vm(i,a)..    zz(j,i,a)=e=gamma(i,a)*xx(j,i,a);
...
zbool_low(i,a)$(doNotFederate(a) and vm(i,a))..      sum(j,zz(j,i,a))- gamma(i,a)*sum(j,xx(j,i,a))=g=K*max_mem(a)*(xfactor(a)-1);
zbool_up(i,a)$(doNotFederate(a)  and vm(i,a))..      sum(j,zz(j,i,a))=l=K*max_mem(a)*xfactor(a);
...
* checking new problem with missing probabilistic functionality
new_admittance(i,a)$vm(i,a)..     nn(i,a)=l=xboolean(i,a) + fed_vms_bool(i,a);
new_admittance_(i,a)$vm(i,a)..    2*nn(i,a)=g=xboolean(i,a) + fed_vms_bool(i,a);
admittance_(a)..                  xfactor_(a)*sum(i$vm(i,a), 1)=e=sum(i$vm(i,a),nn(i,a));
...
* TREC Constraints
eco_cons(a)..    Eco_s(a)*xfactor_(a) =g= eco_constraint(a)*xfactor_(a);

risk_cons(a)..   Risk(a)*xfactor_(a) =l= risk_constraint(a)*xfactor_(a);

trust_cons(a)..  Trust(a)*xfactor_(a) =g= trust_constraint(a)*xfactor_(a);

cost_eq(a)..     cost(a)=e=sum(joff, involved_ph_(joff,a)*z(joff)) + accepted_vms_percent(a)*Pav(a)*(1-average_cdf(a)) + sum(i,fed_cost_per_vm(i,a)*fed_vms(i,a));

cost_low(a)..    cost_constraint(a)-cost(a)=g=L*(cost_bool(a)-1);
cost_up(a)..     cost(a)=l=L*cost_bool(a);

ttt(a)..         accepted_vms_percent(a)=e=cost_bool(a)*accepted_vms_percent(a);

new(i,a)$((1-doNotFederate(a)) and vm(i,a))..  cdf_sc(i,a)=g=av(i,a)*(1-xboolean(i,a))*fed_vms_bool(i,a);

* Objective function

*obj..                 objective=e= sum(a, xfactor_(a)*Gain(a)) - wc*sum(joff, bool(joff)*z(joff)) - wr*sum(a,xfactor_(a)*Pav(a)*(max_av(a)-average_cdf(a)))+
obj..                 objective=e= sum(a, xfactor_(a)*Gain(a)) - wc*sum(joff, bool(joff)*z(joff)) - wr*sum(a,xfactor_(a)*Pav(a)*(1-average_cdf(a)))+
                                   we*sum(j, bool(j)*z(j)*Eco(j)) + we*sum(a, xfactor_(a)*Eco_s(a)) + wt*sum(a, xfactor_(a)*Trust(a)) -
                                   wr*sum(a, xfactor_(a)*Risk(a)) - wc*sum((i,a),fed_cost_per_vm(i,a)*fed_vms(i,a));

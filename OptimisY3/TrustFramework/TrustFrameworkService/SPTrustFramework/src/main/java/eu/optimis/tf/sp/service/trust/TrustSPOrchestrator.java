...
import eu.optimis.tf.sp.service.operators.ExponentialSmoothingAggregator;
import eu.optimis.tf.sp.service.operators.FuzzyAggregator;
import eu.optimis.tf.sp.service.operators.Opinion;
import eu.optimis.tf.sp.service.utils.PropertiesUtils;

public class TrustSPOrchestrator {
	...
	int scale = 1;
	public TrustSPOrchestrator(ArrayList<String> sidList) {
	...
	private void saveServiceInfo(sp2ip spip) {
		...
		double runtime = myEAggregator.calculateAggregation(serviceId, spip.getVmperformance(), ExponentialSmoothingAggregator.RUNTIME);
		double sla = myEAggregator.calculateAggregation(serviceId, spip.getSlaagreement(), ExponentialSmoothingAggregator.SLA);
		...
		try {
			...
			log.error("Error inserting service trust sp2ip");
		}

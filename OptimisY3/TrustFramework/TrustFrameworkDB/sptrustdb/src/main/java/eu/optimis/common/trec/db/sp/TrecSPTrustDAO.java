...
import eu.optimis.trec.common.db.sp.model.SpTrust;

public class TrecSPTrustDAO {
	private final Logger logger = LoggerFactory.getLogger(TrecSPTrustDAO.class
			.getName());
	...
	public boolean deleteIPTrust(String sp) throws Exception {
		...
		try {
			...
			tx.begin();
			Query query = session
					.createSQLQuery("DELETE FROM sp_trust WHERE sp_id =:sp");

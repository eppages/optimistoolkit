...
public class OCCI_RestClient {
    ...
    public String createVM(String service_id, ServiceComponent sc, int index) throws ServiceInstantiationException {
        ...
        try {
        	...
        	JSONObject input = renderCompute(service_id, sc, index);
        	log.info("Creating a VM through OCCI using:");
    ...
    public VMProperties getVM(String serviceId, int index) throws ServiceInstantiationException {
        ...
        {
        	throw new ServiceInstantiationException("There was a problem parsing the VM JSON object", ex);
        }
    }
    ...
    private JSONObject renderCompute(String service_id, ServiceComponent sc, Integer index) throws JSONEncodingException{
        ...
        try {
            ...
            //setAttribute(ServiceComponent.OCCI_COMPUTE_ARCHITECTURE, attrs, "x64");
            setAttribute(ServiceComponent.OCCI_COMPUTE_CORES, attrs, sc.getCores());
            setAttribute(ServiceComponent.OCCI_COMPUTE_MEMORY, attrs, (sc.getMemory()/1024));
            ...
            setAttribute(ServiceComponent.OCCI_COMPUTE_STATUS, attrs, "active");
            setAttribute(ServiceComponent.OCCI_COMPUTE_HOSTNAME, attrs, "10.10.10.10");
            json.put(ATTRIBUTES_TAG, attrs);
            json.put(ID_TAG, service_id + ID_PREFIX + index.toString());
        } catch (JSONException e) {

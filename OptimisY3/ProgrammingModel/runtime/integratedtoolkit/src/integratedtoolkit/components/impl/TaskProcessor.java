...
import integratedtoolkit.types.message.tp.AlreadyAccessedRequest;
import integratedtoolkit.types.message.tp.BlockAndGetResultFilesRequest;
import integratedtoolkit.types.message.tp.CheckObsoleteRequest;
import integratedtoolkit.types.message.tp.DeleteFileRequest;
import integratedtoolkit.types.message.tp.EndOfAppRequest;
import integratedtoolkit.types.message.tp.GetLastDataAccessRequest;
import integratedtoolkit.types.message.tp.GetLastRenamingRequest;
import integratedtoolkit.types.message.tp.GraphDescriptionRequest;
import integratedtoolkit.types.message.tp.GraphUpdateRequest;
import integratedtoolkit.types.message.tp.IsObjectHereRequest;
import integratedtoolkit.types.message.tp.NewVersionSameValueRequest;
import integratedtoolkit.types.message.tp.RegisterDataAccessRequest;
import integratedtoolkit.types.message.tp.SetObjectVersionValueRequest;
import integratedtoolkit.types.message.tp.ShutdownRequest;
import integratedtoolkit.types.message.tp.TPMessage;
import integratedtoolkit.types.message.tp.TaskAnalysisRequest;
import integratedtoolkit.types.message.tp.TasksStateRequest;
import integratedtoolkit.types.message.tp.UnblockResultFilesRequest;
...
public class TaskProcessor implements Runnable, TaskCreation, TaskStatus {
    ...
    // Subcomponents
    private TaskAnalyser TA;
    private DataInfoProvider DIP;
    // Processor thread
    private static Thread processor;
    private static boolean keepGoing;
    // Tasks to be processed
    private LinkedBlockingQueue<TPMessage> requestQueue;
    // Component logger
    ...
    public TaskProcessor(String appHost, String tempDirPath) throws IOException, ClosedChannelException, InterruptedException {
        TA = new TaskAnalyser();
        DIP = new DataInfoProvider(appHost, tempDirPath);

        requestQueue = new Server(NIOConstants.TP_PORT);
            ...
            switch (request.getRequestType()) {
                ...
                case REGISTER_DATA_ACCESS:
                    ...
                    try {
                        ...
                        logger.fatal("Cannot contact the DIP");
                        System.exit(1);
                    } finally {
                ...
                case BLOCK_AND_GET_RESULT_FILES:
                    ...
                    if (writtenDataIds != null) {
                        List<Integer> versionIds = DIP.getLastVersions(writtenDataIds);
                        DIP.blockDataIds(writtenDataIds);
                        resFiles = new ArrayList<ResultFile>(writtenDataIds.size());
                        ListIterator<Integer> iter = versionIds.listIterator();
                        for (Integer dataId : writtenDataIds) {
                            Integer versionId = iter.next();
                            resFiles.add(new ResultFile(new DataInstanceId(dataId, versionId),
                                    DIP.getOriginalName(dataId),
                                    DIP.getOriginalLocation(dataId)));
                        }
                    } else {
                ...
                case GRAPHSTATE:
                    ...
                    break;
                case DELETE_FILE:
                    System.out.println("TP dealing with delete file");
                    DeleteFileRequest dfReq = (DeleteFileRequest) request;
                    FileInfo fileInfo = DIP.deleteData(dfReq.getFileName(), dfReq.getPath(), dfReq.getHost());
                    if (fileInfo != null) { //Involved in any task
                    	System.out.println("FILE ID is : " + fileInfo.getDataId());
                        TA.deleteData(fileInfo);
                    }
                    java.io.File f = new java.io.File(dfReq.getPath() + java.io.File.separator + dfReq.getFileName());
                    f.delete();
                    break;
                case TASKSTATE:
                    ...
                    break;
                case CHECK_OBSOLETE:
                    CheckObsoleteRequest cor = (CheckObsoleteRequest) request;
                    LinkedList<String> obsoletes = new LinkedList();
                    DIP.checkObsolete(cor.getDaId(), obsoletes);
                    TA.obsoleteData(obsoletes);
                    break;
            }
        }
    }
    ...
    // App
    public void newTask(Long appId,
            String methodClass,
            String methodName,
            boolean hasTarget,
            Parameter[] parameters) {
        Task currentTask = new Task(appId, methodClass, methodName, hasTarget, parameters);
    ...
    public void newTask(Long appId,
            ...
            boolean hasTarget,
            Parameter[] parameters) {
        Task currentTask = new Task(appId, namespace, service, port, operation, hasTarget, parameters);
        requestQueue.offer(new TaskAnalysisRequest(currentTask));
    }
    ...
    public static void notifyTaskEnd(Task task) {

        if (debug) {
            logger.debug("Notification received for task " + task.getTaskId() + " with end status " + task.getStatus());
        }
        GraphUpdateRequest ten = new GraphUpdateRequest(task);
        try {
            Client.send(task.getSource(), NIOConstants.TP_PORT, ten);
        } catch (Exception ex) {
            logger.error("Cannot notify task " + task.getSchedulingTaskId() + " end", ex);
        }
    }

    // App
    public void waitForTask(int dataId) {
        Semaphore sem = new Semaphore(0);
        requestQueue.offer(new WaitForTaskRequest(dataId, sem));
    ...
    public void shutdown() {
        requestQueue.offer(new ShutdownRequest());
    }
    ...
    public void checkObsolete(DataInstanceId rdId) {
        CheckObsoleteRequest cor = new CheckObsoleteRequest(rdId);
        requestQueue.offer(cor);
    }
}

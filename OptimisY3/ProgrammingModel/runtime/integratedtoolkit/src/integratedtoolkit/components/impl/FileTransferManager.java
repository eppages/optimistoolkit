...
import integratedtoolkit.types.data.FileOperation.*;
...
public class FileTransferManager implements FileTransfer, DataInfoUpdate {
    ...
    public FileTransferManager() {
        ...
        logger.info("ENV FTM: " + System.getProperty(ITConstants.GAT_ADAPTOR));
        logger.info("Adaptors Name: " + adaptor);
        ...
        nameToPhysicalData = new TreeMap<String, PhysicalDataInfo>();
    ...
    public void setCoWorkers(TaskScheduler TS, JobManager JM) {
        this.TS = TS;
        this.JM = JM;
    }
    ...
    public int getTransferId(int numTransfers, FileRole role) {
        return opGroups.addGroup(numTransfers, role, null);
    }
    ...
    private int transferFiles(
        ...
        if (groupId == null) {
            groupId = opGroups.addGroup(parameters.size(), role, request);
        }
        ...
        if (transfersLeft == 0) {
            try {
                ...
                logger.fatal("Cannot reply request", e);
            }
            return FileTransfer.FILES_READY;
        }
        return groupId;
    ...
    public void transferBackResultFiles(List<ResultFile> resFiles, TransferBackResultFilesRequest request) {
        // Create the group of operations for result files transfer
        int opGId = opGroups.addGroup(resFiles.size(), FileRole.RESULT_FILE, request);
        ...
        for (ResultFile resFile : resFiles) {
            ...
            if (renaming == null) {
                ...
                try {
                    ...
                    logger.error("Cannot reply request", ex);
                }
                return;
            }
    ...
    public void transferFileForOpen(DataAccessId faId, Location targetLocation, TransferFileForOpenRequest request) {
        ...
        if (faId instanceof WAccessId) {
            ...
            list.add(fp);
            transferFiles(null, list, targetLocation, FileRole.OPEN_FILE, request);
        }
    }

    public void transferFileRaw(DataAccessId faId, Location targetLocation, TransferFileRawRequest request) {
        RAccessId raId = (RAccessId) faId;
        DataInstanceId sourceFile = raId.getReadDataInstance();
        int groupId = opGroups.addGroup(1, FileRole.RAW_FILE, request); // Actually we don't care about the group
    ...
    public void deleteIntermediateFiles(DeleteIntermediateFilesRequest request) {
    ...
    public TreeSet<String> getHosts(DataInstanceId dId) {
        ...
        hosts = SharedDiskManager.getAllMachinesfromFile(renaming);
    ...
    void transferObjectValue(TransferObjectRequest toRequest) {
        ...
        if (o == null) {
            ...
            list.add(op);
            transferFiles(null, list, targetLocation, FileRole.OPEN_FILE, toRequest);
        } else {
        ...
        synchronized (inProgress) {
            ...
            if (orderedOperations == 0) {
                ...
                if (inProgress.isEmpty()) {
                    try {
                        //NO MORE TRANSFERS IN PROGRESS --> SHUTDOWN
    ...
    public void obsoleteVersions(LinkedList<String> obsoletes) {
        ...
        for (String renaming : obsoletes) {
            ...
            if (pdi.isInMemory()) {
                LinkedList<String> resourceList = obsoletesMap.get("WSJOB.storage");
                if (resourceList == null) {
                    resourceList = new LinkedList<String>();
                    obsoletesMap.put("WSJOB.storage", resourceList);
                }
            ...
            try {
                synchronized (lf.getURIs()) {
                    for (URI u : lf.getURIs()) {
                        if (u.getHost().compareTo(IntegratedToolkitImpl.appHost) != 0) {
            ...
            synchronized (fOp.getGroupIds()) {
                for (int groupId : groupIds) {
                    ...
                    if (numOps == 0) {
                        ...
                        FileRole fr = null;
                        try {
                            fr = opGroups.removeGroup(groupId);
                        } catch (IOException ex) {
                            logger.error("Cannot reply request", ex);
                            return;
                        }
            ...
            synchronized (fOp.getGroupIds()) {
                for (int groupId : fOp.getGroupIds()) {
                    ...
                    switch (fr) {
                        case JOB_FILE:
                            try {
                                int numOps = opGroups.failedMember(groupId);
                                if (numOps == 0) {
                                    int failedMembers = opGroups.hasFailedMembers(groupId);
                                ...
                                logger.fatal("Can not find transfer group",e );
                                System.exit(1);
                            }
                            break;
                        case OPEN_FILE:
                        ...
                        case SAFE_FILE:
                            try {
                                opGroups.failedMember(groupId);
                            } catch (Exception e) {
                            }
                            logger.fatal(NO_SAFE_COPY + ": File " + fOp.getName()
                                    + ", Role " + fr + "."
                                    + " Operation end state is " + fOp.getEndState(),
                                    fOp.getException());
                            System.exit(1);
                        ...
                        case SHUTDOWN:
                            try {
                                int numOps = opGroups.failedMember(groupId);
                                if (numOps == 0) {
                                    String hostName = group2Host.remove(groupId);
                                    hostName = group2Host.remove(groupId);
                                    stoppingHosts.remove(hostName);
                                    hostToFiles.remove(hostName);
                                    SharedDiskManager.terminate(hostName);

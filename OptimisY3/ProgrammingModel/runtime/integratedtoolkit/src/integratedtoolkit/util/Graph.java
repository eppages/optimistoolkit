...
public class Graph<K, T> {
    ...
    public String getGraphDotFormat() {
        ...
        for (Entry<K, Node<T>> e : graph.entrySet()) {
            ...
            nodes.append(e.getKey()).append("[shape=");
            if (t.getCore() instanceof Method) {
                nodes.append("circle");
            } else {
                nodes.append("diamond");
            }
            nodes.append(", style=filled fillcolor=\"#");
            if (e.getValue().getPredecessors().isEmpty()) {
                nodes.append("00ff00\" fontcolor=\"#000000\"]; \n");
            } else {
                nodes.append("ff0000\" fontcolor=\"#FFFFFF\"]; \n");
            }
            for (T task : e.getValue().getSuccessors()) {
                edges.append(e.getKey()).append(" -> ").append(((Task) task).getTaskId()).append("\n");
            }
        }
     ...
     * @param <E> Represented object type
     */

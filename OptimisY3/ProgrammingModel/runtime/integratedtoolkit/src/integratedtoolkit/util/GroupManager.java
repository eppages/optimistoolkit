...
public class GroupManager {
    ...
    /** Amount of failures produced on operations of each group*/
    private Map<Integer, Message> requests;
    ...
    public GroupManager() {
        ...
        this.requests = new TreeMap<Integer, Message>();
     ...
     */
    public synchronized int addGroup(int numberOfMembers, FileRole fr, Message request) {
        groups.put(nextGroupId, numberOfMembers);
        roles.put(nextGroupId, fr);
        requests.put(nextGroupId, request);
     ...
     */
    public synchronized FileRole removeGroup(int groupId) throws IOException {
        groups.remove(groupId);
        failures.remove(groupId);
        Message request = requests.remove(groupId);
        if (request != null) {
            request.reply(null);
        }

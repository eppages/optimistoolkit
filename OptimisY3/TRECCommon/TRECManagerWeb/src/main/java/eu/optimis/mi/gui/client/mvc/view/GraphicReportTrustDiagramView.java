...
import eu.optimis.mi.gui.client.userwidget.graph.GraphicReportTrustDiagramPanel;

public class GraphicReportTrustDiagramView extends View {

	
	private LayoutContainer container;
	private GraphicReportTrustDiagramPanel grdp;
	
	public GraphicReportTrustDiagramView(Controller controller) {
		super(controller);
	}
	...
	protected void handleEvent(AppEvent event) {
		if (event.getType() == MainEvents.ReportGraphicTrustDiagram) {
			...
			if (eventDataList.size() > 1) {
				...
				List<TrustResourceSP> mrlist = (List<TrustResourceSP>) eventDataList.get(1);
	...
	protected void initialize() {
		...
		grdp = new GraphicReportTrustDiagramPanel();
